...
use crate::stacked::vanilla::utils::CPULock;
...
pub fn get_core_index(core_index: CoreIndex) -> usize {
    return core_index.0
}
            ...
            for (i, group) in groups.iter().enumerate() {
                match group.try_lock() {
                    Ok(guard) => {
                        ...
                        //这里检查锁
                        let c = (*guard)[0];
                        if let Some(_) =  CPULock::lock(c.0, 0) {
                            return Some(guard);
                        }else {
                            drop(guard); //不可用
                        };
...
pub fn bind_core_advance(core_index: CoreIndex, cur_layer: usize, serial_num: usize) -> Result<Cleanup> {
    ...
    //debug!("!!!wcl layer{}-{}, allowed cpuset: {:?}, ", &cur_layer, &serial_num,  cpuset);
    let mut bind_to = cpuset;
    ...
    debug!("!!!wcl layer{}-{}, binding to {:?}", &cur_layer, &serial_num, bind_to);
    // Set the binding.
    let result = locked_topo
        .set_cpubind_for_thread(tid, bind_to, CPUBIND_THREAD)
